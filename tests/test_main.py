...
async def test_run_job_search_success(
    mock_settings, mock_tracker_cls, mock_notif_cls, mock_search_cls
):
    # Setup mocks
    ...
    mock_search.search_all = AsyncMock(
        return_value=[
            Job(
                title="Dev",
                company="Co",
...
@patch("main.ApplicationTracker")
@patch("main.get_settings")
async def test_run_job_search_no_jobs(
    mock_settings, mock_tracker_cls, mock_notif_cls, mock_search_cls
):
    mock_search = mock_search_cls.return_value
